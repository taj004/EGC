...
def gather(gb, 
   ...
   # The divergence
    ...
    # If want to run the script, but on a different grid type  
...
#%% The temperatue equtaion. 
    ...
    specific_heat_cells = to_vec(gb, 
                                  param_kw=temperature, 
                                  param="specific_heat_fluid", 
                                  to_ad=True) 
    specific_heat_faces = to_vec(gb, 
                                  param_kw=temperature, 
                                  param="specific_heat_fluid",
                                  size_type="faces", 
                                  to_ad=True)
    ...
    # Conduction
    conduction_mpfa = pp.ad.TpfaAd(temperature, grids=grid_list)
    ...
    if len(edge_list) > 0:
        # Upstream convection
        convection_coupling = pp.ad.UpwindCouplingAd(keyword=temperature, edges=edge_list)
        ...
        interface_convection = convection_coupling.mortar_discr * w - (
...
#%% Next, the (solute) transport equations.
    ...
    transport = (
        ...
        + all_2_aq.transpose() * div * advection
    ) 

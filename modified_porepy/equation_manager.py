    ...
    def to_ad(
        ...
        for (g, var) in self._dof_manager.block_dof:
            ...
            if isinstance(g, tuple):
               # print(ind.size)
               # print(gb.edge_props(g, pp.STATE)[var].size)
...
class EquationManager:
    ...
    def assemble_rhs(
        ...
        """ Assemble only the residual vector with respect to the current
        state represented in self.gb.
        
        Similar to assemble_matrix_rhs above. 
        Could be useful for line search approaches in Newton method 
        or cyclic variant of Newton  
